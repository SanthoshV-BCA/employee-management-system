...
<div id="snackbar" class="snackbar"></div>
    ...
    ?>
        ...
        <button class="start-task-button" onclick="toggleTask()">Start Task</button>
...
<script>
    var isTaskStarted = false; // Variable to track the task status
    ...
    function startTask() {
        // Set the start time when the task is started
        startTime = new Date();
        isTaskStarted = true;

        // Change the button style and text
        var startButton = document.querySelector('.start-task-button');
        startButton.innerText = "Stop Task";
        startButton.classList.add('stop-task-button');

        alert("Task started!");
    }

    function stopTask() {
        // Reset the task status and change the button style and text
        isTaskStarted = false;
        var startButton = document.querySelector('.start-task-button');
        startButton.innerText = "Start Task";
        startButton.classList.remove('stop-task-button');
    ...
    function completeTask() {
        if (startTime) {
            ...
            var seconds = Math.floor((timeSpent % 60000) / 1000);

            // Display the total work hours in the snackbar
            showSnackbar("Total work hours: " + hours + "h " + minutes + "m " + seconds + "s", "success");
            ...
            // Show an error message if the task has not started
            showSnackbar("Task not started!", "error");
        }
    }
</script>
